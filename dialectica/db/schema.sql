...
alter table public."Dialectica_generations"
  ...
  -- Snippet pipeline (standalone admin job): which kind of run this row is,
  -- and where the per-claim audio-snippet output blob lives. `job_kind`
  -- defaults to 'generation' so every pre-existing run row is untouched; a
  -- 'snippets' run targets an EXISTING map (map_id) rather than building one.
  add column if not exists job_kind text not null default 'generation',
  add column if not exists snippets_path text,
...
-- Add 'quoting' (Stage 5 quote-retrieval) and 'snippeting' (standalone audio-
-- snippet pipeline) to the status check constraint. Drop the auto-named
-- constraint and replace it with the expanded set. The constraint name is
-- auto-generated by Postgres as `Dialectica_generations_status_check` when no
...
alter table public."Dialectica_generations"
  ...
  check (status in ('queued', 'transcribing', 'extracting', 'distilling', 'organizing', 'relating', 'fact_checking', 'quoting', 'snippeting', 'mapping', 'succeeded', 'failed'));

-- Guard the job_kind values. Idempotent drop+add.
alter table public."Dialectica_generations"
  drop constraint if exists "Dialectica_generations_job_kind_check";
alter table public."Dialectica_generations"
  add constraint "Dialectica_generations_job_kind_check"
  check (job_kind in ('generation', 'snippets'));
